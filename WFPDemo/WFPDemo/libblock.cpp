...
DWORD CLibBlock::libblock_init(DWORD init_type, HANDLE* engine_handle)
{
	...
	session.flags = FWPM_SESSION_FLAG_DYNAMIC;		// session结束后自动销毁所有的callout和filter
	...
	if (init_type & LIBBLOCK_INIT_IPV4)
	{
		...
		if (status != ERROR_SUCCESS)
		{
			FwpmTransactionAbort(*engine_handle);		// 终端当前的回话并回滚
			return status;
		}
	...
	if (init_type & LIBBLOCK_INIT_IPV4)
	{
		...
		filter_v4.action.type = FWP_ACTION_CALLOUT_TERMINATING;	//调用始终返回阻止或允许的标注
