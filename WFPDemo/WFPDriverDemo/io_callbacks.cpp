...
VOID device_ioctl(IN WDFQUEUE queue, IN WDFREQUEST request, IN size_t out_length, IN size_t in_length, IN ULONG code)
{
	...
	{
		KdPrint(("[LIBBLOCK]device ioctl IOCTL_GET_DOMAIN"));
		WdfRequestComplete(request, STATUS_SUCCESS);
	}
	else if (code == IOCTL_SET_DOMAIN)
	{
		...
		{
			KdPrint(("[LIBBLOCK]device ioctl get input buffer error:%d", status));
			WdfRequestComplete(request, status);
			return;
		}
		if (inlen != sizeof(rule_t))
		{
			KdPrint(("[LIBBLOCK]device ioctl get input buffer len error:%d", status));
