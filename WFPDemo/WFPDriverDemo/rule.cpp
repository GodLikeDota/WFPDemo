...
NTSTATUS add_rule(rule_t& rule)
{
	...
	if (node == NULL)
	{
		KdPrint(("[LIBBLOCK]failed while executing ExallocatePool2"));
		return STATUS_UNSUCCESSFUL;
	}
	memset(node, 0, sizeof(rule_node_t));
	node->rule = rule;

	KdPrint(("[LIBBLOCK]add rule :add rule domain is %s,action is %d", node->rule.remote_addr_info.domin_str, node->rule.action));
...
{
	KdPrint(("[LIBBLOCK]clear rules"));
	WdfWaitLockAcquire(rule_list_lock, nullptr);
	while (IsListEmpty(&rule_list) == FALSE)
	{
		PLIST_ENTRY node = RemoveTailList(&rule_list);
		rule_node_t* rule_node = CONTAINING_RECORD(node, rule_node_t, list_entry);
		KdPrint(("[LIBBLOCK]clear rules clear %s kernel rule", rule_node->rule.remote_addr_info.domin_str));
